...
class ModelMapper
{
    ...
    public function buildGraph(Collection $models) : Graph
    {
        ...
        foreach (config('modelmap.graph') as $key => $value) {
    ...
    protected function getModelLabel(EloquentModel $model, string $label)
    {
        ...
        $table .= '<tr width="100%"><td width="100%" bgcolor="'.config('modelmap.table.header_background_color').'"><font color="'.config('modelmap.table.header_font_color').'">' . $label . '</font></td></tr>' . PHP_EOL;

        if (config('modelmap.use_db_schema')) {
            $columns = $this->getTableColumnsFromModel($model);
            foreach ($columns as $column) {
                $label = $column->getName();
                if (config('modelmap.use_column_types')) {
                    $label .= ' ('.$column->getType()->getName().')';
                }
                $table .= '<tr width="100%"><td port="' . $column->getName() . '" align="left" width="100%"  bgcolor="'.config('modelmap.table.row_background_color').'"><font color="'.config('modelmap.table.row_font_color').'" >' . $label . '</font></td></tr>' . PHP_EOL;
            }
        }
    ...
    protected function addNodeToGraph(EloquentModel $eloquentModel, string $nodeName, string $label)
    {
        ...
        foreach (config('modelmap.node') as $key => $value) {
            $node->{"set${key}"}($value);
        }
    ...
    protected function connectNodes(Node $modelNode, Node $relatedModelNode, ModelRelation $relation): void
    {
        ...
        foreach (config('modelmap.edge') as $key => $value) {
            $edge->{"set${key}"}($value);
        }

        foreach (config('modelmap.relations.' . $relation->getType(), []) as $key => $value) {
            $edge->{"set${key}"}($value);
        }
